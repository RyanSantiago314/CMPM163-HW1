Shader "Custom/InputEdge"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _LookUpDistance("LookUpDistance", Float) = 0.6
        _mY("MouseY", Float) = 0
        _mX("MouseX", Float) = 0
        _Cells("Cells", Int) = 1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            {
              float mix = 2 * (_mX / _ScreenParams.x) - 1;
              float LookUpDistance = _mY / _ScreenParams.y;
        ...
        float2 texel = float2(
            _MainTex_TexelSize.x * LookUpDistance, 
            _MainTex_TexelSize.y * LookUpDistance
           );
        ...
        float4 edgeCol = lerp(texPix, edgePix, mix); 
        
        float sum = floor(i.uv.x * _Cells) + floor(i.uv.y * _Cells);
